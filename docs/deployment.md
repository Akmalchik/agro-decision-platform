...
Seed добавляет или обновляет тестовый участок по стабильному UUID:

```bash
npm run db:seed
```

Запускайте его вручную только если тестовый участок нужен в production. Seed не запускается во время установки, сборки или deployment.
